...
Small PyTorch timing benchmark (20 epochs):

```sh
pip install -U -r requirements-pytorch.txt
python scripts/train_freerouting.py \
  --framework pytorch \
  --torch-epochs 20 \
  --torch-batch-size 32 \
  --torch-max-boards 128 \
  --data-dir ../DSN \
  --device gpu
```

Random 3-board benchmark sample (prints selected board names):

```sh
python scripts/train_freerouting.py \
  --framework pytorch \
  --torch-epochs 20 \
  --torch-random-boards 3 \
  --data-dir ../DSN \
  --device gpu
```

The PyTorch benchmark keeps DSN strict mode. If `--data-dir` or `--manifest`
is provided but no `.dsn` is loaded, it exits with an error and does not
fallback.

Verification script (checks GPU run + `source : freerouting_boards=3`):

```sh
python scripts/verify_torch_benchmark.py \
  --data-dir ../DSN \
  --random-boards 3 \
  --device gpu
```

PyTorch RL probe with real Freerouting interaction:
